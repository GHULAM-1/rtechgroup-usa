-- Fix security definer issues for views by recreating them properly
-- Drop and recreate view_customer_statements without SECURITY DEFINER
DROP VIEW IF EXISTS public.view_customer_statements;
CREATE VIEW public.view_customer_statements AS
SELECT 
  le.customer_id,
...
-- Drop and recreate view_fines_export without SECURITY DEFINER
DROP VIEW IF EXISTS public.view_fines_export;
CREATE VIEW public.view_fines_export AS
SELECT 
  f.id as fine_id,
  f.reference_no,
  f.type,
